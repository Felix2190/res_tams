...
<head>
    ...
    <title>descuentos cruz roja</title>
...
<body> 
            ...
            <div id="content" class="clearfix">
                ...
                <header id="header-sec"> 
                	<div class="inner-padding"> 
                        <div class="pull-left">
                            <h2><i class="fa fa-tags"></i> &nbsp; Descuentos cruz roja</h2>                 
                        </div> 
                    </div>
            	</header>
                    ...
                    <div class="row ext-raster">
                        ...
                        <div class="col-sm-12">
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-12" id="descuentoscruz">
                                	<div class="inner-padding">
	...
	SELECT cms_site,amountdd,Amountmd , CASE  WHEN amountdd-Amountmd=0 THEN 'total' ELSE 'parcial' END AS Tipo 
	FROM (
	...
	WHEN '10' THEN 'San Quintin' WHEN '12' THEN 'Rec Palaco' ELSE 'Otros' END AS cms_site,amountdd,Amountmd,TotalPaid
	FROM 
	(SELECT  ta.IdPayDocument, GUID,[DATEADD],tb.Amount AS amountdd,td.Amount Amountmd,TotalPaid
	FROM    mercurioprod.dbo.PayDocument TA
		...
		INNER JOIN mercurioprod.dbo.ManualDiscount TD
			ON TD.IdPayDocumentDetail = TB.IdPayDocumentDetail
		WHERE [DATEADD] between @fini AND @ffin AND tb.IdPaymentItem=33
	) AS PER1
	...
	GROUP BY IdPayDocument,cms_snum,TextData,TextDatacms,amountdd,Amountmd,TotalPaid
	) AS descuentos

INSERT INTO @Resul 
	SELECT CASE oficina WHEN 'Cd Morelos' THEN 'Mexicali' WHEN 'Mariano' THEN 'Tijuana' WHEN 'San Felipe' THEN 'Mexicali' WHEN 'Via Rapida' THEN 'Tijuana'Else oficina END AS Ciudad,
	oficina,sum(aportacion) AS importe,COUNT(*) aS [cantidad donativos],tipo,1 AS orden,0 AS ordeni    FROM @descuentos	
WHERE oficina IN ( ";
														...
														if ($mexicali=='checked')
												     	...
												     	$tsql=trim($tsql,',').") GROUP BY oficina,tipo ORDER BY 1,oficina,tipo
						...
						<div><div class="inner-padding"><div class="col-sm-1">
									<a href="javascript:imprSelec('descuentoscruz')" class="btn btn-info btn-lg"><span class="glyphicon glyphicon-print"></span> Imprimir</a>
								</div>
								<div class="col-sm-1">
									&nbsp;
								</div>
