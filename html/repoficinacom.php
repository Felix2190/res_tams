...
<head>
    ...
    <title>Comparativo por oficina</title>
            ...
            <div id="content" class="clearfix">
                ...
                <header id="header-sec"> 
                	<div class="inner-padding"> 
                        <div class="pull-left">
                            <h2><i class="fa fa-map-marker"></i> &nbsp; Comparativo por oficina</h2>                 
                        </div> 
                    </div>
            	</header>
											...
											<table class="table table-bordered table-striped" id="tb1" data-rt-breakpoint="600">
												<thead>
													<tr>
														...
														<td scope="col" data-rt-column="Project">A&ntilde;o</td>
                                                                                               ...
                                                                                               FROM          (SELECT     a1.cms_CardId, a2.cms_status, a2.cms_holder, a1.cms_event, a1.cms_FromDate, LEFT(CONVERT(varchar, 
                                                                                                                       ...
                                                                                                                       FROM          (SELECT     CardId AS cms_CardId, 
                                                                                                                                                                      CASE CardHistEventID WHEN 1 THEN 'Activate' WHEN 3 THEN 'Revoke' ELSE 'Desc' END AS cms_event,
                                                                                                                                                                       FromDate AS cms_FromDate
                                                                                                                                               FROM          EPICMS.dbo.CardHist
                                                                                                                                               WHERE      CONVERT(char(10),FromDate,120)    BETWEEN CONVERT(char(10),DATEADD (yyyy,-1,CAST(@fini AS DATETIME)),120) AND CONVERT(char(10),DATEADD (yyyy,-1,CAST( @ffin AS DATETIME)),120)
                                                                                                                                                                      ) AS a1 LEFT OUTER JOIN
                                                                                                                                                  (SELECT     CardID AS cardid, 
                                                                                                                                                                           CASE Status WHEN 5 THEN 'Inactive' WHEN 7 THEN 'Active' ELSE 'Desc' END AS cms_status, 
                                                                                                                                                                           CardHolderUID AS cms_holder
                                                                                               ...
                                                                                               FROM          (SELECT     a1.cms_CardId, a2.cms_status, a2.cms_holder, a1.cms_event, a1.cms_FromDate, LEFT(CONVERT(varchar, 
                                                                                                                       ...
                                                                                                                       FROM          (SELECT     CardId AS cms_CardId, 
                                                                                                                                                                      CASE CardHistEventID WHEN 1 THEN 'Activate' WHEN 3 THEN 'Revoke' ELSE 'Desc' END AS cms_event,
                                                                                                                                                                       FromDate AS cms_FromDate
                                                                                                                                               FROM          EPICMS.dbo.CardHist
                                                                                                                                               WHERE      CONVERT(char(10),FromDate,120)    BETWEEN CONVERT(char(10),DATEADD (yyyy,-2,CAST(@fini AS DATETIME)),120) AND CONVERT(char(10),DATEADD (yyyy,-2,CAST( @ffin AS DATETIME)),120)
                                                                                                                                                                      ) AS a1 LEFT OUTER JOIN
                                                                                                                                                  (SELECT     CardID AS cardid, 
                                                                                                                                                                           CASE Status WHEN 5 THEN 'Inactive' WHEN 7 THEN 'Active' ELSE 'Desc' END AS cms_status, 
                                                                                                                                                                           CardHolderUID AS cms_holder
...
SELECT Ciudad,sitio,lustro , ISNULL([Expedicion],0) [Expedicion],ISNULL([Revalidacion],0) [Revalidacion],ISNULL([Reposicion],0) [Reposicion],ISNULL([Garantia],0) AS [Garantia]
,ISNULL([Expedicion],0)+ISNULL([Revalidacion],0)+ISNULL([Reposicion],0)+ISNULL([Garantia],0)AS total
FROM(
	SELECT tramite,'Mexicali' Ciudad ,sitio,COUNT(totales) AS total,DATEPART(yyyy,fecha) AS lustro 	
	FROM @resultados
	WHERE sitio IN ('Cd Morelos','Mexicali','Gpe Victoria','San Felipe','Rec Palaco')
	GROUP BY tramite,sitio,DATEPART(yyyy,fecha)
	UNION 
	SELECT tramite,'Tijuana' Ciudad ,sitio,COUNT(totales) AS total,DATEPART(yyyy,fecha) AS lustro 	
	FROM @resultados
	WHERE sitio IN ('Via Rapida','Mariano')
	GROUP BY tramite,sitio,DATEPART(yyyy,fecha)
	UNION 
	SELECT tramite,'Ensenada' Ciudad ,sitio,COUNT(totales) AS total,DATEPART(yyyy,fecha) AS lustro 	
	FROM @resultados
	WHERE sitio IN ('Ensenada','San Quintin')
	GROUP BY tramite,sitio,DATEPART(yyyy,fecha)
	UNION 
	SELECT tramite,'Tecate' Ciudad ,sitio,COUNT(totales) AS total	,DATEPART(yyyy,fecha) AS lustro 		
	FROM @resultados
	WHERE sitio IN ('Tecate')
	GROUP BY tramite,sitio,DATEPART(yyyy,fecha)
	UNION 
	SELECT tramite,'Rosarito' Ciudad ,sitio,COUNT(totales) AS total,DATEPART(yyyy,fecha) AS lustro 	
	FROM @resultados
