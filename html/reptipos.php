...
<head>
    ...
    <title>Reporte por tipos de licencia</title>
            ...
            <div id="content" class="clearfix">
                ...
                <header id="header-sec"> 
                	<div class="inner-padding"> 
                        <div class="pull-left">
                            <h2><i class="fa fa-truck"></i> &nbsp; Tipos de licencia</h2>                 
                        </div> 
                    </div>
            	</header>
                    	...
                    	<div class="col-sm-12" >
                            <form action="<?php echo $_SERVER['PHP_SELF']; ?> " method="post" name="form">
                							...
                							<div class="col-sm-4">
                								...
                								<input type="text" data-date-format='yy-mm-dd' name="txtFechaFin" id="txtFechaFin" class="form-control txtFechaFin" value="<?php echo $fechafin; ?>" />
                							</div> 											
                                	...
                                	<div class="inner-padding">
                                		<div class="table-wrapper">
											...
											<table class="table table-bordered table-striped" id="tb1" data-rt-breakpoint="600">
													...
													<tr>
														<td scope="col" data-rt-column="ID">Ciudad</th>
														<td scope="col" data-rt-column="Project">Oficina</th>
														<td scope="col" data-rt-column="Status">Automovilista
														<td scope="col" data-rt-column="Status">Motociclista</th>														
														<td scope="col" data-rt-column="Progress">Chofer A</th>
														<td scope="col" data-rt-column="Progress">Chofer B</th>
														<td scope="col" data-rt-column="Progress">Chofer C</th>
														<td scope="col" data-rt-column="Progress">Chofer D</th>
														<td scope="col" data-rt-column="Progress">Prov. Auto.</th>
														<td scope="col" data-rt-column="Progress">Prov. Chofer C.</th>
														<td scope="col" data-rt-column="Progress">Total</th>
													</tr>
...
SET @fini='".$fechaini."'
SET @ffin='".$fechafin."'
DECLARE @Resul TABLE(ciudad VARCHAR(30),Sitio VARCHAR(30),automovilista INT, Motociclista INT, 	ChoferA INT, ChoferB INT,ChoferC INT,ChoferD INT,ProvAuto INT,ProvChoferC INT,Total INT,ordeni INT,orden INT)
                       ...
                       FROM          (SELECT     last_month_cms_cardid, e.cms_month, e.cms_day, e.cms_status, e.cms_event, e.cms_site, e.cms_tot, e.cms_tramite, f.cms_owner, 
                                               ...
                                               FROM          (SELECT     c.cms_CardId AS last_month_cms_cardid, c.cms_month, c.cms_day, c.cms_status, c.cms_holder, c.cms_event, 
                                                                                              CASE cms_snum WHEN '01' THEN 'Gpe Victoria' WHEN '02' THEN 'Cd Morelos' WHEN '03' THEN 'San Felipe' WHEN '04' THEN 'Mexicali'
                                                                                               ...
                                                                                               WHEN '10' THEN 'San Quintin' WHEN '12' THEN 'Rec Palaco' ELSE 'Otros' END AS cms_site, c.cms_tot, c.cms_tramite, 
...
INSERT INTO @resul
SELECT Ciudad,sitio,ISNULL([MOTO],0) [Motociclista],ISNULL([AUTO],0) [Automovilista],ISNULL([CHFA],0) [Chofer A],ISNULL([CHFB],0) AS [Chofer B]
,ISNULL([CHFC],0) AS [Chofer `C],ISNULL([CHFD],0) AS [Chofer D],ISNULL([PRAU],0) AS [Provisional Automovilista],ISNULL([PRCC],0) AS [Provisional Chofer C]
,ISNULL([MOTO],0)+ISNULL([AUTO],0)+ISNULL([CHFA],0)+ISNULL([CHFB],0) + ISNULL([CHFC],0)+ ISNULL([CHFD],0)+ ISNULL([PRAU],0)+ISNULL([PRCC],0) AS total
,1 AS orden,0 AS ordeni   
FROM(
	SELECT tipo,'Mexicali' Ciudad ,sitio,SUM(totales) AS total	
	FROM @resultados
	WHERE sitio IN ('Cd Morelos','Mexicali','Gpe Victoria','San Felipe','Rec Palaco')
	GROUP BY tipo,sitio
	UNION 
	SELECT tipo,'Tijuana' Ciudad ,sitio,SUM(totales) AS total
	FROM @resultados
	WHERE sitio IN ('Via Rapida','Mariano')
	GROUP BY tipo,sitio
	UNION 
	SELECT tipo,'Ensenada' Ciudad ,sitio,SUM(totales) AS total
	FROM @resultados
	WHERE sitio IN ('Ensenada','San Quintin')
	GROUP BY tipo,sitio
	UNION 
	SELECT tipo,'Tecate' Ciudad ,sitio,SUM(totales) AS total		
	FROM @resultados
	WHERE sitio IN ('Tecate')
	GROUP BY tipo,sitio
	UNION 
	SELECT tipo,'Rosarito' Ciudad ,sitio,SUM(totales) AS total
	FROM @resultados
...
PIVOT (
	SUM(total) 
	FOR tipo IN ([AUTO],[CHFB],[CHFA],[CHFC],[CHFD],[MOTO],[PRAU],[PRCC],[MEDE])
	) AS pvt
WHERE ciudad IN ( ";
														...
														WHILE @@FETCH_STATUS = 0  
															BEGIN  
																INSERT INTO @Resul
																SELECT ciudad,'Total', SUM(automovilista),SUM(Motociclista),SUM(ChoferA),SUM(ChoferB)
																,SUM(ChoferC),SUM(ChoferD),SUM(ProvAuto),SUM(ProvChoferC),SUM(Total) ,1 ,1
														...
														INSERT INTO @Resul
														SELECT 'Total','',SUM(automovilista),SUM(Motociclista),SUM(ChoferA),SUM(ChoferB)
																,SUM(ChoferC),SUM(ChoferD),SUM(ProvAuto),SUM(ProvChoferC),SUM(Total),2,2 FROM @Resul 
														WHERE Sitio<>'Total'
