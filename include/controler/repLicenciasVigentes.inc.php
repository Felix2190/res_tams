<?php
 ...
 // for($i=0; $i < count($arrAnios); $i++){
  	...
  	foreach ( $rega as $key => $registro ) {//contiene las localidades como key y los datos como $registros
  		$objPHPExcel->setActiveSheetIndex ( 0 )->setCellValue ( $indicesTabla [0] . $fila, date("Y", strtotime($fi)) );//año
...
//   		print_r($FechaF);
  	foreach($ubica As $idUbicacion=>$nombre){	
  		...
  		/////////////////AUTOMOVILISTA/////////////////
  		$query=" select count(*)AS total from  licencia where idUbicacion = $idUbicacion and idTipoLicencia in(1,2,3) and estatus = 'aprobada' and fechaExpiracion  between DATE_ADD(DATE('".$FechaIni."'), INTERVAL 1 DAY) and DATE_ADD(DATE('".$FechaFin."'), INTERVAL 1 DAY)";//  echo $query;
  		...
  		/////////CHOFER/////////
  		$query=" select count(*)AS total from  licencia where idUbicacion = $idUbicacion and idTipoLicencia in(4,5,6) and estatus = 'aprobada' and fechaExpiracion  between DATE_ADD(DATE('".$FechaIni."'), INTERVAL 1 DAY) and DATE_ADD(DATE('".$FechaFin."'), INTERVAL 1 DAY)
  		";//  echo $query;
  		mysqli_query($dbLink,"SET NAMES 'utf8'");// solucion en linux
  		$result=mysqli_query($dbLink,$query);
  		if(!$result)
  			...
  			///////////////MOTOCICLISTA/////////////////
  			$query=" select count(*)AS total from  licencia where idUbicacion = $idUbicacion and idTipoLicencia in(7,8,9) and estatus = 'aprobada' and fechaExpiracion  between DATE_ADD(DATE('".$FechaIni."'), INTERVAL 1 DAY) and DATE_ADD(DATE('".$FechaFin."'), INTERVAL 1 DAY)
  			";//  echo $query;
  			mysqli_query($dbLink,"SET NAMES 'utf8'");// solucion en linux
  			$result=mysqli_query($dbLink,$query);
  			if(!$result)
 ...
 /////////////////// Tipos de licencia menor de edad       **************************************************************************
  				...
  				fechaExpiracion between DATE_ADD(DATE('".$FechaIni."'), INTERVAL 1 DAY) and DATE_ADD(DATE('".$FechaFin."'), INTERVAL 1 DAY) and  				
  	...
  	$tablaExp .="<div class='table-wrapper'><header> Reporte de Licencias vigentes. Fecha inicio:".$FechaIni.", Fecha fin:".$FechaFin."</header> </div><table class='table'>
					<thead>
  	...
  	foreach($tipos as $key=>$value){  	
  		...
  		<td>".date("Y", strtotime($FechaIni))."</td>
  		<td>$key</td>
  		<td>".$value['menor']."</td>	
