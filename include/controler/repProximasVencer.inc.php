<?php
 ...
 // for($i=0; $i < count($arrAnios); $i++){
  	...
  	$b =  " Reporte Licencias Proximas a vencer. Fecha inicio: ". $fi."-".$arrAnios[$i] ." - ". $ff ." Fecha fin: ".$arrAnios[$i];
  	$objPHPExcel->setActiveSheetIndex ( 0 )->mergeCells('A'.$fila.':H'.$fila)->setCellValue ("A" . $fila,$b );
  	$objPHPExcel->getActiveSheet()->setSharedStyle($estiloTabla1,"A" . $fila);
  	$fila++;
...
//   		print_r($FechaF);
  	foreach($ubica As $idUbicacion=>$nombre){	
  		...
  		/////////////////AUTOMOVILISTA/////////////////
  		$query=" select count(*)AS total from  licencia where idUbicacion = $idUbicacion and idTipoLicencia in(1,2,3) and estatus = 'aprobada' and fechaExpiracion  between DATE('".$FechaIni."') and DATE('".$FechaFin."')";//  echo $query;
  		...
  		/////////CHOFER/////////
  		$query=" select count(*)AS total from  licencia where idUbicacion = $idUbicacion and idTipoLicencia in(4,5,6) and estatus = 'aprobada' and fechaExpiracion  between DATE('".$FechaIni."') and DATE('".$FechaFin."')
  		";//  echo $query;
  		mysqli_query($dbLink,"SET NAMES 'utf8'");// solucion en linux
  		$result=mysqli_query($dbLink,$query);
  		if(!$result)
  			...
  			///////////////MOTOCICLISTA/////////////////
  			$query=" select count(*)AS total from  licencia where idUbicacion = $idUbicacion and idTipoLicencia in(7,8,9) and estatus = 'aprobada' and fechaExpiracion  between DATE('".$FechaIni."') and DATE('".$FechaFin."')
  			";//  echo $query;
  			mysqli_query($dbLink,"SET NAMES 'utf8'");// solucion en linux
  			$result=mysqli_query($dbLink,$query);
  			if(!$result)
 ...
 /////////////////// Tipos de licencia menor de edad       **************************************************************************
  				...
  				fechaExpiracion between DATE('".$FechaIni."') and DATE('".$FechaFin."') and  				
  	...
  	$tablaExp .="<div class='table-wrapper'><header> Reporte Licencias Proximas a vencer. Fecha inicio:".$FechaIni.", Fecha fin:".$FechaFin."</header> </div><table class='table'>
					<thead>
