<?php
	...
	function registrar($idTurno, $txtFolioTesoreria, $txtComentarios,$forma,$ultimos,$enviarBanco=false)
	{
		...
		if($total<0)
			...
			$r->mostrarAviso("Se almacen&oacute; la informaci&oacute;n de manera correcta.");
			$r->redirect("turnoPagoImpresion.php");
	...
	function buscarTurnoPago($idInterno, $idExterno)
	{
		...
		if($existe)
		{
			...
			$r->mostrarAviso("El pago para este tr&aacute;mite ya est&aacute; registrado.<br />Folio recaudaci&oacute;n: <strong>" . $Pago->getFolioRecaudacion() . "</strong><br /><a href=\"dashboard.php\" class=\"link\">Ir a Dashboard</a>");
			$r->call("removerInformacion");
			$r->redirect("dashboard.php",5);
			return $r;
		}
	...
	if(isset($_SESSION['turno'])&&ctype_digit(trim($_SESSION['turno'])))
	{
		$idTurnoActual=$_SESSION['turno'];
	}
	else
	{
