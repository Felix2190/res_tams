<?php
	...
	require_once FOLDER_MODEL . "extend/model.persona_biometrico.inc.php";
	        ...
	        if (!$Municipios->getError ())
	            foreach ( $arrMun as $cvMunicipios => $nombre )
	                ...
	                if ($datosTab2['localidad']!=''){
	                    ...
	                    $arrLoc = $localidades->getCiudadesByEntidadMunicipio( $datosTab2['estado'], $datosTab2['municipio'] );
...
//	    echo $_SESSION['idContacto'].'<br />';
	...
	// datos biometricos
	$biometricos= new ModeloPersona_biometrico();
	
	$arrBiometricos=$biometricos->bioHuellaByIdPersona($_SESSION['idPersonaBio']);
	$slcBiometricos ='';
	$contador=0;
	foreach ( $arrBiometricos as $clave => $nombre ){
	    if ($contador==0)
	        $idBiom=$clave;
	    $slcBiometricos .= '<option value="' . $clave . '"  >' . $nombre . '</option>';
	    $contador++;
	}
	//echo $slcBiometricos.'<br />';
...
//	    $inegiDom->setColonia($txtColonia);
	...
	function avanzaSeccion($seccion,$txtTurno) {
	    ...
	    if (intval($seccion)==5){
	        ...
	        $r->redirect("biometricos.php?id=" . $txtTurno,2);
	        
	    }else {
	    $_SESSION['_biograficos']= $seccion;
	    
	    $r->redirect("biograficos.php?id=" . $txtTurno,2);
	    }
	    return $r;
...
function cambioMunicipio($cveEstado, $cveMunicipio)
{
    ...
    $Localidades = new ModeloPostales($dbLink );
		...
		$arrLocalidades = $Localidades->getCiudadesByEntidadMunicipio( $cveEstado, $cveMunicipio );
		if ($Localidades->getError ()) {
